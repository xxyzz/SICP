...
;; =====================================================
;; Part of Metacircular evaluator
;; =====================================================
...
(compile
 '(define (factorial-alt n)
    (if (= n 1)
        1
        (* n (factorial-alt (- n 1)))))  ;; switch argument order
 'val
 'next)
'((env)
  (val)
  ...
  ((assign val
   ...
   entry1  ;; calls to factorial-alt will enter here
   (assign env (op compiled-procedure-env) (reg proc))
   (assign env
           (op extend-environment)
           (const (n))
   ...
   (assign argl (op list) (reg val))
   (restore proc)  ;; restore factorial-alt
   ;; apply factorial-alt
   (test (op primitive-procedure?) (reg proc))
   ...
   after-if5
   after-lambda2
   ;; assign the procedure to the variable factorial-alt
   (perform (op define-variable!)
            (const factorial-alt)
            (reg val)
            (reg env))
   (assign val (const ok))))
;; factorial-alt compiles (factorial-alt (- n 1)) before n
;; it add a save of env but remove a save of argl
